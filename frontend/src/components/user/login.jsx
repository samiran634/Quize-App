  ...
  setIsDisabled(true);
    try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/login`, {
            ...
            body: JSON.stringify({ userEmail: userEmail, passward: userPassword }),
            credentials: 'include'
        });
        ...
        if (response.ok) {
            ...
            alert(data.message || "Invalid credentials. Please try again or create an account.");
            setIsDisabled(false);
        }
    } catch (err) {
       ...
       <LeftSection/>
        ...
        <div className="md:w-[52%] p-10 sm:p-14 lg:p-20 flex flex-col justify-center">
          ...
          <p className="text-[#8c8c97] text-md mb-10">
            Do not have an account? <a href="/signup" className="text-[#a492f2] hover:text-[#c4bdf5] underline underline-offset-4 decoration-1 font-medium transition-colors">Create One</a>
          </p>
          ...
          <form className="space-y-5" >
            ...
            <button onClick={handleLogin}
              ...
              className="w-full bg-[#785ef0] hover:bg-[#654ae0] text-white font-medium rounded-xl px-5 py-4 mt-8 transition-colors shadow-lg shadow-[#785ef0]/20 cursor-pointer"
